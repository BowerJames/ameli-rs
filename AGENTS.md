...
- `cargo clippy` must pass with no warnings or errors on source code. Test code is exempt.
...
pi-coding-agent has a broad UI provider that handles rendering, commands, and interactive features. **ameli-agent defines an `Interface` trait with a deliberately reduced scope.** This is a deliberate design decision.
...
- The `ameli-agent` crates ship with **no default tools**. This is by design — it is agnostic to what tools an agent needs.
